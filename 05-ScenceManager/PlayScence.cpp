	...
	case OBJECT_TYPE_BRICK:
		{
			...
			obj = new CBrick(x,y,width,height);
		}
		break;
...
void CPlayScene::Update(DWORD dt)
{
	...
	if (cx >= SCREEN_WIDTH_SCENE)
	{
		cx = SCREEN_WIDTH_SCENE;
	}
	if (cy < 0)
	{
		cy = 0;
	}
	if (player->y >= game->GetScreenHeight() + HEIGHT_SCEEN_VALUE)
	{
...
void CPlayScenceKeyHandler::OnKeyDown(int KeyCode)
{
	...
	case DIK_SPACE:
		...
		if (mario->Power == POWER_RUNNING && mario->GetLevel()== MARIO_LEVEL_BIG_TAIL)
		{
